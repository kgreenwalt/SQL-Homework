...
-- 1b. Display the first and last name of each actor in a single colmumn in upper case letters. Name hte column Actor Name
...
-- queries on a description, so create a column in the table `actor` named `description` and use the data type `BLOB` (Make sure to research the type `BLOB`, as the difference between it and `VARCHAR` are significant).
ALTER TABLE `sakila`.`actor` 
ADD COLUMN `description` 
BLOB NULL AFTER `full_name`;	
...
SELECT DISTINCT last_name, count(last_name) as total  from actor group by last_name;

-- 4b. List last names of actors and the number of actors who have that last name, but only for names that are shared by at least two actors
SELECT DISTINCT last_name, count(last_name) as total from actor group by last_name having total 
